...
class FirebaseData {
  ...
  static func getUser(with userID: String, completion: @escaping (User?) -> ()) {
    ...
    userKey.observeSingleEvent(of: .value, with: { (snapshot) in
      guard let userDict = snapshot.value as? [String : Any] else { print("error unwrapping user snapshot"); return }
      
      guard let firstName = userDict["firstName"] as? String else { print("error unwrapping user first name"); return }
      guard let lastName = userDict["lastName"] as? String else { print("error unwrapping user last name"); return }
      
      var reviewsArray = [String]()
      
      guard let reviewsDictionary = userDict["reviews"] as? [String : Any] else { print("error unwrapping user reviews dict"); return }
      ...
      completion(User(userID: userID, firstName: firstName, lastName: lastName, reviewsID: reviewsArray))
    })
  }
